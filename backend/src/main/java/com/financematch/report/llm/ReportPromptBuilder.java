 ...
 *
 * <p>금융자산 축은 규칙이 아직 팀 확정 전(GAP-10)이라 여기 없다.
 */
@Component
public class ReportPromptBuilder {
    ...
    private static final String DEBT_REPAYMENT_RULES =
            ...
            - 둘 다 부채 있음 + 위험도 동일 → "두 분 모두 부채 위험도가 높아요." 형태 (이름 언급 금지)
            - 둘 다 부채 있음 + 위험도 차이 있음 → 위험도가 더 높은 쪽 이름만 언급
              ("두 분 모두 부채가 있어요. 하지만 부채 위험도는 OOO님이 더 높아요." 형태)
            - 한 명만 부채 있음 → "두 분 중 부채가 있는 분이 있어요." (누구인지 이름 언급 금지)
            """;
    ...
    private static final String FINANCIAL_VALUE_RULES_TEMPLATE =
            ...
            - 그 외에는 → "두 분은 {diff 최소 항목}은 비슷하지만 {diff 최대 항목}이 차이가 나요."
              (최소·최대가 동점이면 위 고정 순서에서 먼저 오는 항목을 택한다)
