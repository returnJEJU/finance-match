...
public class ReportPromptBuilder {
    ...
    private static final String FINANCIAL_VALUE_RULES_TEMPLATE =
            ...
            - 각 항목의 |me-partner| 차이(diff)를 기준으로, diff가 %d 이하면 "비슷한 항목",
              %d 초과면 "다른 항목"으로 분류한다.
            - 4개 항목이 전부 비슷한 항목이면 → "두 분은 투자 가치관이 비슷해요."
            - 다른 항목만 있고 비슷한 항목이 하나도 없으면 → "두 분은 {다른 항목 전부를 순서대로
              ·로 이어붙임}이 차이가 나요."
            - 비슷한 항목과 다른 항목이 섞여 있으면 → "두 분은 {비슷한 항목들을 ·로 이어붙임}은
              비슷하지만, {다른 항목들을 ·로 이어붙임}은 차이가 나요."
            - 항목을 나열할 때는 반드시 위 고정 순서(총자산 중 금융자산 비중 > 투자 경험 > 금융 투자
              상품 이해도 > 손실 감내력)를 지킨다.
    ...
    private static final String TAX_STRATEGY_RULES =
            ...
            3. 전부 개설했지만 일부 계좌가 납입 한도 미달이면 → 그 계좌만 지목해 "올해 {계좌명} 한도
               {금액}" 형태로 명시하고(계좌 잔액이 아니라 올해 납입 한도라는 걸 "올해"로 분명히 할 것)
