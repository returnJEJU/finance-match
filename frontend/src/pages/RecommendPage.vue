...
<template>
  <section class="min-h-[calc(100vh-108px)] bg-white px-4 pt-5 pb-10">
    <div v-if="isPending" class="flex min-h-[460px] flex-col items-center justify-center">
      ...
      <p class="mt-3 text-sm text-muted">맞춤 상품을 준비하고 있어요</p>
    </div>
    ...
    >
      ...
      <button
        v-if="!isNotReady"
        type="button"
        class="mt-5 inline-flex items-center gap-2 rounded-xl bg-ink px-5 py-3 text-xs font-semibold text-white"
        @click="refetch()"
      >
    ...
    <template v-else-if="recommendation">
      ...
      <div
        class="mt-4 transition duration-300"
      ...
      <p class="mt-10 text-center text-[9px] leading-4 text-muted-soft">
