...
- **화면 여백 (잠정)**: 기준 프레임 `max-w-[428px]`(iPhone 12 Pro Max), 좌우 `px-7`, 화면 하단 주 버튼 아래 `pb-7`. 높이는 `min-h-dvh`를 쓴다 — `min-h-screen`(=`100vh`)은 브라우저 주소창·하단바가 없다고 가정한 높이라 실제 보이는 영역보다 커서, 바닥에 둔 버튼이 화면 밖으로 밀린다. **아직 확정이 아니다.** 전체 UI 정리 작업에서 바뀔 수 있으니 새 화면은 일단 이 값을 따르고, 기준이 바뀌면 그때 한 번에 맞춘다.
